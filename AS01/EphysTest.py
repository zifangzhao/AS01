...
# show current path
file = 'D:\\code\\Python\\data\\20170429_laser_170429_101010.dat'
...
Len = fs * 100
Nch = 32
data = np.fromfile(file, dtype=np.int16, count=Nch * Len, offset=0)
data = np.reshape(data.T, (-1, 32))
print(data.size)

plt.plot(data - 1000 * np.array(range(32)))

from scipy import signal as sg

data_lfp = sg.resample_poly(data, 1250, 20000);
plt.plot(data_lfp - 1000 * np.array(range(32)))
plt.title('Resampled')
plt.savefig('Resampled.eps')

data_lfp.astype('int16').tofile('Resampled.dat')
...
class nsxfile:
    def __init__(self, filename):
        ...
        if (self.FileTypeID == 'NEURALSG'):
            ...
            # data=np.fromfile(filename,dtype=np.int16,count=batchsize+overlap,offset=offsets[0])
        else:
    ...
    def ReadData(self, offset, length):  # offset is in bytes, length is in samples
        ...
        while length > 0:  # loop until read enough bytes
            ...
            print('offset in section:' + str(offset_in_section) + 'offset in file:' + str(
                offset_in_file) + "cur_section:" + str(curr_section) + " Length:" + str(length))
    ...
    def ConvertToDat(self, res_fs=20000, filename_output=''):
        if filename_output is '':
...
# file='D:\\code\\Python\\data\\pc2504005_matlab.dat'
# Resampling(file,32,30000,20000)

# nfile=nsxfile('D:\\code\\python\\data\\20101117-rat6-restrained-30samplerate-laser+007.ns5')
nfile = nsxfile('D:\\code\\python\\data\\PC2504005.ns6')
nfile.ConvertToDat()
